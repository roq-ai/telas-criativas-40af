...
import { deleteArtworkById } from 'apiSdk/artworks';
...
function VendorViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<VendorInterface>(
    ...
    () =>
      getVendorById(id, {
        relations: ['user', 'artwork'],
      }),
  );

  const artworkHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('artwork', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Artworks
                    </Text>
                    <NextLink passHref href={`/artworks/create?vendor_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.artwork?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/artworks/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
